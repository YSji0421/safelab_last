...
import { analyzeConsultation, askAboutClause } from '../services/gemini';
...
export default function ConsultationRoomPage() {
  ...
  // 약관 챗봇 — STT 발화가 멎으면 누적된 질문을 Gemini 로 보냄
  ...
  const speakAsAvatar = useCallback((text) => {
    ...
    speak(text, {
      ...
      onEnd: () => {
        setIsSpeaking(false);
        setCurrentUtterance('');
        // 발화 종료 후 짧은 cooldown — STT echo 잡기 방지
        setTimeout(() => { isSpeakingRef.current = false; }, 800);
      }
    });
    setTranscript(prev => [...prev, { speaker: '🤖 AI', text, time: new Date().toLocaleTimeString('ko-KR'), isAi: true }]);
  }, []);

  // 약관 챗봇 트리거 — 누적된 STT 질문을 Gemini 에 보내고 음성으로 답변
  const triggerClauseChat = useCallback(async (question) => {
    if (isChattingRef.current) return;
    if (isSpeakingRef.current) return; // AI 발화 중이면 echo 가능성 — skip
    isChattingRef.current = true;
    try {
      const result = await askAboutClause(question);
      if (result?.answer) {
        const clauseSuffix = result.clauseRef ? ` (${result.clauseRef})` : '';
        setTranscript(prev => [...prev, {
          speaker: '🤖 AI',
          text: result.answer + clauseSuffix,
          time: new Date().toLocaleTimeString('ko-KR'),
          isAi: true,
        }]);
        speakAsAvatar(result.answer);
      }
    } catch (e) {
      console.warn('[ChatBot] 응답 실패:', e?.message || e);
    } finally {
  ...
  const onTranscript = useCallback((text) => {
    ...
    // 약관 챗봇 자동 응답 — 발화가 1.6초 멎으면 누적 질문을 Gemini 로
    if (isSpeakingRef.current || isChattingRef.current) return; // AI 발화/응답 중이면 echo 회피
    ...
    chatDebounceRef.current = setTimeout(() => {
      const q = pendingChatRef.current.trim();
    ...
    <div className="room-wrap">
      ...
      <main className="panel-video">
        ...
        <div className="ctrl-bar">
          <button
            className="ctrl-btn emergency-btn"
