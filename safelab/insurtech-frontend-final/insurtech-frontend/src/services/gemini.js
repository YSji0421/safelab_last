...
// gemini-1.5-flash 는 2025년 9월 deprecated. 2.5-flash 가 현재 안정 버전.
const GEMINI_MODEL = process.env.REACT_APP_GEMINI_MODEL || "gemini-2.5-flash";
const GEMINI_URL = `https://generativelanguage.googleapis.com/v1beta/models/${GEMINI_MODEL}:generateContent?key=${GEMINI_API_KEY}`;

const callGemini = async (prompt, { temperature = 0.4, maxOutputTokens = 1500, json = true } = {}) => {
  ...
  if (!res.ok) {
    ...
    console.warn('[Gemini callGemini] HTTP', res.status, errText.slice(0, 200));
    return '';
  }
  const data = await res.json();
  // parts 배열 전체에서 text 가 있는 part 만 추출 (thinking part 등 제외)
  const parts = data.candidates?.[0]?.content?.parts || [];
  const text = parts.map(p => p?.text).filter(Boolean).join('') || '';
  if (!text) {
    console.warn('[Gemini callGemini] 빈 응답 — block reason:', data.promptFeedback || data.candidates?.[0]?.finishReason);
  }
...

{
  ...
  try {
    const res = await fetch(GEMINI_URL, {
      ...
      body: JSON.stringify({
        ...
        generationConfig: {
          ...
          responseMimeType: "application/json",   // 마크다운 wrapping 방지 — JSON 만 강제
          thinkingConfig: { thinkingBudget: 0 },  // 2.5-flash thinking 토큰 끔 (응답 형식 안정화)
        },
      }),
    });
    if (!res.ok) {
      const errText = await res.text().catch(() => '');
      console.warn('[Gemini analyzeConsultation] HTTP', res.status, errText.slice(0, 200));
      return { ...getMockAnalysis(transcript), source: 'fallback' };
    }
    const data = await res.json();
    // parts 배열 전체에서 text 가 있는 part 만 추출 (thinking part 등 제외)
    ...
    if (!rawText) {
      console.warn('[Gemini analyzeConsultation] 빈 응답 — block reason:', data.promptFeedback || data.candidates?.[0]?.finishReason);
      return { ...getMockAnalysis(transcript), source: 'fallback' };
    }
    const parsed = safeJsonParse(rawText, null);
    if (!parsed) {
      console.warn('[Gemini analyzeConsultation] JSON parse 실패 — 원문:', rawText.slice(0, 200));
      return { ...getMockAnalysis(transcript), source: 'fallback' };
    }
    return { ...parsed, source: 'ai' };
  } catch (err) {
    console.warn('[Gemini analyzeConsultation] 예외:', err?.message || err);
    return { ...getMockAnalysis(transcript), source: 'fallback' };
  }
};

// ─────────────────────────────────────────────────────────
// 약관 챗봇 — 학생 발화에 약관 컨텍스트 기반으로 짧게 답변
// ConsultationRoomPage 의 STT 자동 트리거에서 사용
// ─────────────────────────────────────────────────────────
export const askAboutClause = async (question) => {
  ...
  if (!isGeminiConfigured()) {
    return {
      source: 'fallback',
      answer: '죄송해요, AI 약관 답변이 일시적으로 어렵습니다. 안전관리실 또는 한국교육시설안전원으로 문의 부탁드립니다.',
    };
  }

  const prompt = `당신은 인하공전 학생 단체보험 AI 상담사 "진아"입니다. 따뜻하고 정확한 어조로 답하세요.
아래 약관 컨텍스트만을 근거로 학생 질문에 답합니다. 컨텍스트에 없는 내용은 추측하지 말고 "약관 발췌에는 해당 내용이 없으니 안전관리실에 확인 부탁드립니다"라고 답하세요.
...
답변 규칙:
- 한국어로 1~3문장. 음성으로 발화되니 자연스럽게.
- 가능하면 근거 조항 번호("제3조", "별표1" 등) 명시.
- 금액/기간 같은 숫자는 정확히 인용.
- 약관에 없는 사항은 추측 금지.

JSON만 출력:
{
  "answer": "학생에게 들려줄 답변 문장",
  "clauseRef": "근거 조항 번호 (없으면 빈 문자열)"
}`;

  try {
    const text = await callGemini(prompt, { temperature: 0.4, maxOutputTokens: 600 });
    if (!text) {
      return {
        source: 'fallback',
        answer: '죄송해요, 답변을 불러오지 못했어요. 다시 한 번 말씀해 주시겠어요?',
      };
    }
    const parsed = safeJsonParse(text, null);
    if (!parsed?.answer) {
      console.warn('[Gemini askAboutClause] JSON 파싱 실패 — 원문:', text.slice(0, 200));
      return {
        source: 'fallback',
        answer: '죄송해요, 답변을 불러오지 못했어요. 다시 한 번 말씀해 주시겠어요?',
      };
    ...
    console.warn('[Gemini askAboutClause] 예외:', err?.message || err);
    return {
      source: 'fallback',
      answer: '죄송해요, 일시적으로 답변이 어려워요. 잠시 후 다시 시도해 주세요.',
    };
  }
};
